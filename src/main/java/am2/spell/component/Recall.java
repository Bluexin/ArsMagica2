	...
	public boolean applyEffectEntity(ItemStack stack, World world, EntityLivingBase caster, Entity target){
		...
		if (caster.isPotionActive(PotionEffectsDefs.astralDistortion) || ((EntityLivingBase)target).isPotionActive(PotionEffectsDefs.astralDistortion)){
			if (caster instanceof EntityPlayer)
				((EntityPlayer)caster).sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.tooltip.cantTeleport")));
			return false;
		}
		...
		if (casterProperties.getMarkDimensionID() == -512){
			if (caster instanceof EntityPlayer && !world.isRemote)
				((EntityPlayer)caster).sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.tooltip.noMark")));
			return false;
		}else if (casterProperties.getMarkDimensionID() != caster.dimension){
			if (caster instanceof EntityPlayer && !world.isRemote)
				((EntityPlayer)caster).sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.tooltip.diffDimMark")));
			return false;
		}
		if (!world.isRemote){
	...
	private boolean handleRitualReagents(ItemStack[] ritualRunes, World world, BlockPos pos, EntityLivingBase caster, Entity target){
		...
		if (!hasVinteumDust && ritualRunes.length == 3){
			...
			if (vector == null || vector.equals(new AMVector3(pos))){
				if (caster instanceof EntityPlayer && !world.isRemote)
					((EntityPlayer)caster).sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.tooltip.noMatchingGate")));
				return false;
			}else{
			...
			if (player == null){
				if (caster instanceof EntityPlayer && !world.isRemote)
					((EntityPlayer)caster).sendStatusMessage(new TextComponentString("am2.tooltip.noMatchingPlayer"));
				return false;
			}
			else if (player == caster){
				if (caster instanceof EntityPlayer && !world.isRemote)
					((EntityPlayer)caster).sendStatusMessage(new TextComponentString("am2.tooltip.cantSummonSelf"));
				return false;
			}else{
				RitualShapeHelper.instance.consumeAllReagents(this, world, pos);
