...
public class GuiMagiciansWorkbench extends GuiContainer{
	...
	protected void drawGuiContainerBackgroundLayer(float f, int mouseX, int mouseY){
		...
		if (this.workbenchInventory.getUpgradeStatus(TileEntityMagiciansWorkbench.UPG_CRAFT))
		...
		//tabs
		Iterator<RememberedRecipe> it = this.workbenchInventory.getRememberedRecipeItems().iterator();
		int n = 0;
		recipeIndex = -1;
		hoverRecipeIndex = -1;
	...
	protected void drawHoveringText(List<String> par1List, int par2, int par3, FontRenderer font, int color){
		if (!par1List.isEmpty()){
