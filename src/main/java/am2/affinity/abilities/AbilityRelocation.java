...
public class AbilityRelocation extends AbstractAffinityAbility {
	...
	public void applyKeyPress(EntityPlayer player) {
		...
		if (MinecraftForge.EVENT_BUS.post(event)) {
			if (!player.world.isRemote)
				player.sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.chat.relocation_failed")));
			return;
		}
		...
		if (player.getDistanceSq(event.getTargetX(), posY, event.getTargetZ()) > 1024) {
			if (!player.world.isRemote)
				player.sendStatusMessage(new TextComponentString(I18n.translateToLocal("am2.chat.relocation_out_of_range")));
			return;
		}
